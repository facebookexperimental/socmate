    ...
    for conn in connections:
        ...
        if src_port.width != dst_port.width:
            mismatches.append(IntegrationMismatch(
                ...
                suggested_fix=(
                    f"Widen the narrower port or add explicit "
                    f"truncation/extension in the top-level wiring"
                ),
                details={
...
def generate_top_level_rtl(
    ...
    # Build module header
    lines: list[str] = []
    lines.append(f"// Auto-generated top-level integration module")
    lines.append(f"// Design: {design_name}")
    lines.append(f"// Blocks: {len(modules)}")
    lines.append(f"// Generated by socmate integration pipeline")
    lines.append(f"")
